...
class CustomEnvironment(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self._target_location = np.array([7, 3])
    ...
    def step(self, action):
        ...
        if self._agent_location[0] in (3, 4, 5, 8):
            self._agent_location = np.clip(
                self._agent_location + np.array([0, -1]), 0, self.size - 1
            )

        if self._agent_location[0] in (6, 7):
            self._agent_location = np.clip(
                self._agent_location + np.array([0, -2]), 0, self.size - 1
            )

        self._agent_location[1] = np.clip(
            self._agent_location[1], 0, 6
        )
